...
export default function RecurringDepositPage() {
  ...
  const onOpen = async (values) => {
    ...
    try {
      ...
      setSuccess('Recurring deposit opened.');
      reset();
      load();
    } catch (err) {
      setError(extractError(err, 'Could not open RD'));
    }
  };

  const rds = list.filter((d) => d.depositType === 'RECURRING');
            ...
            <CardContent>
              ...
              <form onSubmit={handleSubmit(onOpen)}>
                <Stack spacing={2} sx={{ mt: 1 }}>
                  <TextField
                    label="Monthly amount (min 100)"
                    type="number"
                    inputProps={{ step: '0.01', min: '100' }}
                  ...
                  <TextField
                    label="Tenure (months)"
                    type="number"
                    inputProps={{ step: '1', min: '6', max: '120' }}
          ...
          <TableContainer component={Paper}>
            <Table size="small">
              ...
              <TableBody>
                ...
                {rds.map((d) => (
                  <TableRow key={d.id}>
                    <TableCell align="right">{d.monthlyAmount}</TableCell>
